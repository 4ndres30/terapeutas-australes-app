...
QA-006 ya cuenta con plan base documental, QA-006A deja matriz de rutas y superficies por rol, QA-006B ejecuta validacion visual de navegacion sin sesion y autenticada local/demo, QA-006C revalida post-merge la navegacion filtrada por rol, QA-006D valida Reportes/Finanzas por rol, QA-006E valida estados Auth minimos local/demo con mensajes no tecnicos y QA-006F revalida RLS/Storage local por rol. BE-023 deja definido documentalmente que Finanzas debe evolucionar hacia alias/codigo financiero persistente no derivado del UUID clinico y sin `paciente_id` visible por defecto. BE-025 define la whitelist de campos financieros permitidos/prohibidos para evitar textos clinicos en Finanzas. CTRL-012 cierra documentalmente el bloque QA/Finanzas. SEC-006 define la politica documental de fotos, retencion y objetos huerfanos, sin habilitar fotos reales. QA-003 valida local/demo una imagen ficticia en Storage privado, metadatos en `fotos_elementos_caso`, visualizacion UI para admin y bloqueo para Finanzas, con observacion sobre carga directa por input de archivo no automatizada. SEC-011 define el diseno documental del hardening tecnico futuro de fotos/Storage: inventario de grants, migracion local, auditoria, anulacion logica, objetos huerfanos y QA posterior. La lectura estatica confirma proteccion de rutas por `admin`, `terapeuta` y `finanzas`; la ejecucion visual confirma redireccion a `/login` sin sesion, redireccion por rol para rutas no autorizadas, reportes por rol, filtrado visible de sidebar/drawer, separacion Finanzas/Reportes y bloqueo controlado para usuario inactivo o sin perfil interno. QA-006F confirma localmente que `finanzas` no accede a clinica, fotos ni Storage `elementos-caso`, que `terapeuta` no accede a cobros/pagos directos y que el bucket sigue privado. SEC-007B agrega herramienta local-only y ejecuta provisioning local de usuarios ficticios, sin `.env`, sin remoto y sin credenciales versionadas. UI-023 queda integrada por PR #74 y validada post-merge: Terapeuta no ve Finanzas y Finanzas no ve superficies clinicas ni Agenda en sidebar o drawer movil.

AUDIT-2026-07-04 audito estructura React, migraciones y RLS, y propuso el roadmap de 5 bloques con DEC-036 a DEC-039 (DEC-040 reservada), aprobado por Javier. `BLOQUE-1-RLS` deja las 3 migraciones RLS corregidas y separadas en `fix/rls-vista-cobros-finanzas`, `fix/rls-fotos-auditoria-finanzas` y `fix/rls-delete-policies`, pendientes de validacion local y PR. `BLOQUE-2-UTIL` deja `src/lib/constants.ts`/`format.ts`/`queries.ts` extraidos correctamente en `refactor/extract-utilities`, con la migracion de imports en paginas aun pendiente. `BLOQUE-3-AUTH` deja el POC de `AuthContext` en `poc/auth-context` validado tecnicamente, pendiente de validacion visual. Los 3 bloques quedaron mergeados a `main` (PRs #85-#90, ver LOG-076..081); este parrafo describia un estado anterior.
...
Actualizacion 2026-07-11: el repositorio paso a visibilidad publica. El primer reintento de
QA-013 expuso un bloqueo de facturacion de la cuenta; el segundo completo `CI / Quality gate`
en el run `29139105668`, con instalacion, lint, 24 tests y build exitosos. QA-013 queda cerrada
con CI remoto exitoso. Branch protection permanece sin activar y preparada para aplicacion
manual. La siguiente tarea recomendada es UI-047, en una rama separada.
...
- UI-048/UI-049/UI-050: propuestas de densidad y aprovechamiento de espacio en `PacientesPage` y el shell interno registradas durante revision local/demo. Siguen pendientes, sin codigo en este PR; por compartir `App.tsx` y sus capas CSS, UI-049 debe implementarse y validarse antes de UI-050.
...
La implementacion futura depende de validar clinica/legalmente BE-020, implementar separacion real de ambientes, implementar auditoria sensible, probar backup/restauracion y cerrar PROD-001.
...
- matriz de permisos;
- hardening Auth;
- backup/restauracion documentado por BE-019/DOC-002, pendiente prueba real;
- indicador visual de ambiente y bloqueo de produccion no habilitada implementados localmente por UI-020/UI-021 y validados por QA-009 en local/demo;
- consentimiento informado;
- auditoría de cambios sensibles;
- política de datos demo vs reales;
- checklist pre-producción.

QA-006 queda iniciado como plan base documental para ordenar pruebas por rol y no exposicion sensible. QA-006A agrega matriz de rutas/superficies, QA-006B confirma redireccion a login sin sesion y redireccion/autorizacion por rol con usuarios ficticios locales. UI-023 corrige la observacion de navegacion visible por rol en local/demo. QA-006D valida Reportes/Finanzas por rol y QA-006E confirma estados Auth minimos con mensajes no tecnicos, pero la validacion progresiva sigue sin habilitar uso real por dependencias de seguridad/productizacion y PROD-001.
...
Este avance no cambia el bloqueo PROD-001: QA-003 queda ejecutada local/demo con observacion; antes de cualquier uso con datos reales aun se requiere atender observaciones SEC-001, aplicar hardening tecnico derivado de SEC-006 y definir auditoria tecnica.
